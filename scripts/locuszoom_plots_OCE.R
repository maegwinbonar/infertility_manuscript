...
for (i in 1:length(temp)) {
  ...
  for (j in 1:length(gene_list)) {
    ...
    #save it
    loc_df[[j]]<- c("gene"=loc$gene, "chr"=loc$seqname, "start"=loc$xrange[1], "end"=loc$xrange[2])
    # plot it
    ...
    ## if there is no rsid found within 50kb of the genes (there was only one case of this)
    if (nrow(var) > 0){
      pl <- quote({
...
# Make a df of the genes and locations
loc_df2 <- do.call(rbind, loc_df)
write.csv(loc_df2, "locuszoom_genes.csv")
